class Job {
    constructor(id, title, description, location, company, salary_min, salary_max, created, category) {
        this.id = id;
        this.title = title;
...
fetch('http://localhost:8080/api/v1/jobs') 
    ...
    .then(data => {
        ...
        latestJobs = allJobs.filter(job => {
            const jobDate = new Date(job.created);
            return jobDate >= startOfLastTwoWeeks; 
        });
    ...
    function displayJobs(jobsArray) {
        ...
        jobsArray.forEach(job => {
            ...
            document.addEventListener('mousemove', function(event) {
                ...
                const popover = document.getElementById('popover-Job');
                const rectButton = exploreButton.getBoundingClientRect();
                const rectPopover = popover.getBoundingClientRect();
            
                const isOutsideButton = event.clientX < rectButton.left || event.clientX > rectButton.right || event.clientY < rectButton.top || event.clientY > rectButton.bottom;
                const isOutsidePopover = event.clientX < rectPopover.left || event.clientX > rectPopover.right || event.clientY < rectPopover.top || event.clientY > rectPopover.bottom;
    ...
    document.getElementById('searchBtn').addEventListener('click', function(e) {
        ...
        const searchMetier = document.getElementById('metier-search').value.toLowerCase();
        const searchRegion = document.getElementById('region-search').value.toLowerCase();
        const searchSecteur = document.getElementById('secteur-search').value.toLowerCase();
    
        const filteredJobs = allJobs.filter(job => 
            job.description.toLowerCase().includes(searchMetier) &&
            job.location.toLowerCase().includes(searchRegion) &&
            job.category.toLowerCase().includes(searchSecteur)
        );
        document.getElementById("tmp").textContent = '';
